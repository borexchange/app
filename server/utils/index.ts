...
export function sendResponse(
  ...
  res.cork(() => {
    ...
    if (status === 'success') {
      // Write secure cookies if tokens are present in the request
      if (
        req &&
      ...
      // Set content type and build the response object
      const response = {
        status: 'success',
        data: {
          result: data || null,
...
export const setupRouteHandler =
  (route: any, controllers: any) => async (res: any, req: any) => {
    ...
    try {
      ...
      const { params, query, body, error } = await extractData(
        ...
        req,
        context,
        calculateParamIndices(route.path),
      )
