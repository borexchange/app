...
export default class ExchangeConnectionManager {
  ...
  async watchData(
    ...
    while (subscription.isActive) {
      try {
        ...
        this.messageBuffer[type][symbol] = data
  ...
  collectClientData(clientHandler: ClientHandler) {
    ...
    for (const type in clientHandler.subscriptions) {
      for (const symbol of clientHandler.subscriptions[type]) {
        if (this.messageBuffer[type] && this.messageBuffer[type][symbol]) {
          clientData[type] = clientData[type] || {}
          clientData[type] = this.messageBuffer[type][symbol]
        }
      }
    }
  ...
  getSubscription(symbol: string, type: string) {
  ...
  removeSubscription(symbol: string, type: string) {
    const key = `${symbol}-${type}`
