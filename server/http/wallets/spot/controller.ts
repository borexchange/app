...
export async function verifyTransaction(userId: number, trx: string) {
  ...
  const settingsObject = Object.fromEntries(
    parsedSettings.map((s) => [s.key, s.value]),
  )
...
export async function generateWalletAddressQuery(currency: string) {
  ...
  if (!connection) throw new Error('Exchange connection failed')
  ...
  switch (provider) {
    ...
    case 'kucoin':
      ...
      if (currencyData) {
        ...
        if (chains.length > 1 && currency !== 'BNB') {
          // Run all address generation promises in parallel
          const promises = chains.map((chain) => {
            const chainName = exchange.safeString(chain, 'chain').toUpperCase()
            return fetchCreateDepositAddress(exchange, currency, chainName)
              .then((address) => {
                results[chainName] = address
              })
          ...
          const chain = exchange.safeValue(chains, 0)
          const chainName = exchange.safeString(chain, 'chain')
          try {
            const address = await fetchCreateDepositAddress(
              exchange,
              currency,
      ...
      for (const key in results) {
        if (results[key].info) {
          results[key] = {
            currency: results[key].currency,
            address: results[key].address,
            tag: results[key].tag,
            network: results[key].network,
          }
        }
      }
      break
